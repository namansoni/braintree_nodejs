...
router.get('/checkouts/new', (req, res) => {
  gateway.clientToken.generate({}).then(({ clientToken }) => {
    ...
    // });
    res.send(JSON.stringify(clientToken))
  });
});
...
router.post('/checkouts', (req, res) => {
  ...
  console.log("checkouts ke andar hoon bhai");
  ...
  gateway.transaction
    ...
    .catch(({ errors }) => {
      ...
      // res.redirect('checkouts/new');
      //res.send(formatErrors(errors));
      res.send(JSON.stringify("PAYMENT FAIL HO GAYI HAI"))
    });
});
